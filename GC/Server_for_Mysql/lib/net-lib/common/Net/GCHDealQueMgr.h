...
struct GCSTH_RecvDataInfo
{
	...
	int					nLen;
	char				buf[PACKET_ONE_LENGTH];
};

class CGCHDealQueMgr
{
public:
	static BOOL Init(void);
	static BOOL Destroy(void);

	static BOOL InsertWaitDealData(void *handle,BYTE yFrameFlag,WORD wCRC16, GCEM_OPERATE_TYPE enOpType, int nLen, char* pBuf);
